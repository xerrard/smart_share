...
public class QSTileView extends ViewGroup {
    ...
    public QSTileView(Context context) {
        ...
        // SPRD: fixbug434024 Set the String for the two line display.
    ...
    protected void setIcon(ImageView iv, QSTile.State state) {
        if (!Objects.equals(state.icon, iv.getTag(R.id.qs_icon_tag))) {
            ...
            if(state.icon == ResourceIcon.get(R.drawable.ic_qs_4g_on_sprd)
            ||state.icon == ResourceIcon.get(R.drawable.ic_qs_4g_off_sprd)){d.setAutoMirrored(false);}
            else{d.setAutoMirrored(true);}
            }
