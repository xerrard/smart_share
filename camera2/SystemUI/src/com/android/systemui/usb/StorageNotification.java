...
public class StorageNotification extends SystemUI {
    ...
    /* SPRD: fixbug421982 update the notification when the local changed @{ */
    private static String sPath = "";
    private static String sOldState = "";
    private static String sNewState = "";
    /* @} */
    ...
    //SPRD : fixbug455319 Add judgment, the phone has SD, do not send the SD card has been removed.
    public final String SDPath = "/storage/sdcard0";
    private static final int STORAGE_DEVICE = 0;
    ...
    private class StorageNotificationEventListener extends StorageEventListener {
        ...
        public void onStorageStateChanged(final String path,
            ...
            mAsyncEventHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    sNewState = newState;
                    /* @} */
                    updateStorageLocation(newState);
     ...
     private void updateStorageLocation (String newState){
	 if(newState.equals(Environment.MEDIA_SHARED)
		...
		|| newState.equals(Environment.MEDIA_BAD_REMOVAL)){
	     int selectStation = Settings.Global.getInt(mContext.getContentResolver(),Settings.Global.DEFAULT_STORAGE_LOCATION, STORAGE_DEVICE);
	     if(selectStation == STORAGE_SDCARD){
		 Settings.Global.putInt(mContext.getContentResolver(),Settings.Global.DEFAULT_STORAGE_LOCATION, STORAGE_DEVICE);
	     }
	 }
    }
    ...
    private void onStorageStateChangedAsync(String path, String oldState, String newState) {
        ...
        if (newState.equals(Environment.MEDIA_SHARED)) {
            ...
            for (StorageVolume volume: volumeList) {
                if (volume.getDescriptionId() == com.android.internal.R.string.storage_usb && volume.getPath().equals(path)){
                    isUSBStorage = true;
                }
            }
            setMediaStorageNotification(
                    //com.android.internal.R.string.ext_media_checking_notification_title,
                    isUSBStorage? com.android.internal.R.string.ext_media_checking_notification_title_1 : com.android.internal.R.string.ext_media_checking_notification_title,
            ...
            setMediaStorageNotification(
                    ...
                    com.android.internal.R.drawable.stat_notify_sdcard_usb, true, false, pi);
            updateUsbMassStorageNotification(mUmsAvailable);
         /* SPRD: fix bug482766 Don't send notification when the phone don't has SD card @{ */
        //SPRD : fixbug455319 Add judgment, the phone has SD, do not send the SD card has been removed.
//        } else if (newState.equals(Environment.MEDIA_REMOVED) && path.equals(SDPath)) {
            /*
...
//            updateUsbMassStorageNotification(false);
    ...
    void updateUsbMassStorageNotification(boolean available) {
        ...
        if (available || (!orignal_support && mUmsAvailable)) {
            Intent intent = new Intent();
            if(orignal_support){
                intent.setClass(mContext, com.android.systemui.usb.UsbStorageActivity.class);
            } else {
    ...
    private synchronized void setUsbStorageNotification(int titleId, int messageId, int icon,
        ...
        if (visible) {
            ...
            if (POP_UMS_ACTIVITY_ON_CONNECT && !adbOn) {
                ...
                //SPRD: fixbug437249 Messages sent through HeadsUpNotificationView are not prohibited.
    ...
    private final BroadcastReceiver mLocalChangeReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            if (true) {
                Log.d(TAG, "language is change....");
                Log.d(TAG, "sPath =" + sPath + " , sOldState =" + sOldState + ", sNewState ="
                        + sNewState);
            }
            onStorageStateChangedAsync(sPath, sOldState, sNewState);
            updateUsbMassStorageNotification(mUmsAvailable);
        }
    };
}
