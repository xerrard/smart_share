...
public class PowerNotificationWarnings implements PowerUI.WarningsUI {
    ...
    private static final String TAG_NOTIFICATION_TEMP = "temp";
    private static final int ID_NOTIFICATION_TEMP = 101;
    ...
    private boolean mPlaySoundForTemp = true;

    private boolean mTempOverLowWarning;
    private boolean mTempOverHighWarning;
    ...
    private void updateNotification() {
        ...
        if (DEBUG) Slog.d(TAG, "temperature updateNotification mTempOverHighWarning=" + mTempOverHighWarning + " mTempOverLowWarning="
                + mTempOverLowWarning );
        if(mTempOverHighWarning){
            showTempOverHighWarningNotification();
        } else if(mTempOverLowWarning){
            showTempOverLowWarningNotification();
        } else {
            mNoMan.cancelAsUser(TAG_NOTIFICATION_TEMP, ID_NOTIFICATION_TEMP, UserHandle.ALL);
        }

    }

    private void showTempOverLowWarningNotification() {
        final Notification.Builder nb = new Notification.Builder(mContext)
                .setSmallIcon(R.drawable.stat_sys_battery_fail)
                .setWhen(0)
                .setShowWhen(false)
                .setOngoing(false)
                .setContentTitle(
                        mContext.getString(R.string.temperature_overlow_title))
                .setContentText(
                        mContext.getString(R.string.temperature_overlow_text))
        ...
        if (mPlaySoundForTemp){
            attachTempBatteryWarningSound(nb);
            mPlaySoundForTemp = false;
        }
        final Notification n = nb.build();
        if (n.headsUpContentView != null) {
            n.headsUpContentView.setViewVisibility(
                    com.android.internal.R.id.right_icon, View.GONE);
        }
        mNoMan.notifyAsUser(TAG_NOTIFICATION_TEMP, ID_NOTIFICATION_TEMP, n,
                UserHandle.ALL);
    }

    private void showTempOverHighWarningNotification() {
        final Notification.Builder nb = new Notification.Builder(mContext)
                .setSmallIcon(R.drawable.stat_sys_battery_fail)
                .setWhen(0)
                .setShowWhen(false)
                .setOngoing(false)
                .setContentTitle(
                        mContext.getString(R.string.temperature_overhigh_title))
                .setContentText(
                        mContext.getString(R.string.temperature_overhigh_text))
                .setPriority(Notification.PRIORITY_MAX)
                .setVisibility(Notification.VISIBILITY_PUBLIC)
                .setColor(
                        mContext.getResources()
    ...
    private void attachTempBatteryWarningSound(Notification.Builder b) {
        final ContentResolver cr = mContext.getContentResolver();
    ...
    @Override
    public void dismissTempOverLowWarning() {
        mTempOverLowWarning = false;
        mPlaySoundForTemp = true;
        updateNotification();
    }

    @Override
    public void dismissTempOverHighWarning() {
        mTempOverHighWarning = false;
        mPlaySoundForTemp = true;
        updateNotification();
    }

    @Override
    public void showTempOverLowWarning() {
        mTempOverLowWarning = true;
        updateNotification();
    }

    @Override
    public void showTempOverHighWarning() {
        mTempOverHighWarning = true;
        updateNotification();
    }
