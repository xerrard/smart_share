...
/*[FACTORY_TEST_BY_APK] modify yujinwang start*/
import java.io.File;
import java.io.FileDescriptor;
import java.io.FileInputStream;
import java.io.FileNotFoundException;
import java.io.IOException;
/*[FACTORY_TEST_BY_APK] modify yujinwang end*/
...
public class KeyguardViewMediator extends SystemUI {
    ...
    private static boolean hideLockSound = true;// add by jiangguohu for grsture lock sound for HQ01654155
    ...
    //add by jiangguohu for gesture start
    ...
    private static final String MA_UNLOCK ="ma.fprint.match.UNLOCK";//add by microarray
    ...
    private void setupLocked() {
        ...
        //add  by jiangguohu start for gesture start
        IntentFilter filter = new IntentFilter();
        filter.addAction(DELAYED_KEYGUARD_ACTION);
        filter.addAction(HQ_HIDE_KEYGUARD) ;
		
        if(com.huaqin.common.featureoption.FeatureOption.HQ_FINGERPRINT_MA){
            filter.addAction(MA_UNLOCK);//add by microarray
        }
        mContext.registerReceiver(mBroadcastReceiver, filter);
        //add  by jiangguohu start for gesture end
        ...
        synchronized (this) {
            ...
            if (shouldWaitForProvisioning()) {
                ...
                if (!mExternallyEnabled) {
                    //taojinjing 20160222 HQ01711208 due to home invalided add
                    /*SPRD: The home&recent key no response@{*/
                    Log.d(TAG,"keyguard is disable,update inputrestricted state");
                    mNeedToReshowWhenReenabled = false;
                    updateInputRestricted();
                    /* @} */
     ...
     */
    public boolean isInputRestricted() {
        if (DBG) Log.i(TAG, "test_ isInputRestricted mShowing =" + mShowing + ";mNeedToReshowWhenReenabled=" +
             mNeedToReshowWhenReenabled + "; shouldWaitForProvisioning()=" + shouldWaitForProvisioning());
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (HQ_HIDE_KEYGUARD.equals(intent.getAction())) { //add by jiangguohu start for gesture
                Log.w(TAG, "HQ_HIDE_KEYGUARD:    " + intent.getAction());
                //hideLocked() ;
                Bundle bundle = intent.getExtras();
                //huanghonglin,[HQ01574187],2016-01-25
                if(bundle != null){
                    hideLockSound = bundle.getBoolean("playLockedSound");
                    Log.i(TAG,"hideLockSound ------>" + hideLockSound);
                }
                if (!isSecure()  && mShowing) {
                    Log.w(TAG, "HQ_HIDE_KEYGUARD11111:    " + intent.getAction());
                    keyguardDone(true, true);
                } 
                //add by jiangguohu END for gesture
            }else if((com.huaqin.common.featureoption.FeatureOption.HQ_FINGERPRINT_MA)
                &&(MA_UNLOCK.equals(intent.getAction()))){//add by microarray 
              	keyguardDone(true,true);
            ...
            } //add by xerrard END for FprintMA
        }
    };
    ...
    private void handleStartKeyguardExitAnimation(long startTime, long fadeoutDuration) {
        synchronized (KeyguardViewMediator.this) {
            ...
            if (TelephonyManager.EXTRA_STATE_IDLE.equals(mPhoneState) && hideLockSound) {// add by jiangguohu for HQ01654155
                playSounds(false);
            }
