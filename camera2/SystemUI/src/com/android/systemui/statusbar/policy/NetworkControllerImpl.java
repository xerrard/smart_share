...
import com.android.internal.telephony.TelephonyIntentsSprd;
...
public class NetworkControllerImpl extends BroadcastReceiver
    ...
    private static boolean isRelianceBoard = "reliance".equals(SystemProperties.get(
            "ro.operator.volte", ""));

    // add wifi in and out icon when WIFI_SIGNAL_OPTIMIZED is true
    private static final boolean WIFI_SIGNAL_OPTIMIZED = SystemProperties.getBoolean("ro.wifi.signal.optimized", true);
  //SPRD: MODIFY FOR BUG 456545
    private static final String ACTION_MODEM_ASSERT = "com.android.modemassert.MODEM_STAT_CHANGE";
    ...
    final Map<Integer, MobileSignalController> mMobileSignalControllers =
            new LinkedHashMap<Integer, MobileSignalController>();
    ...
    NetworkControllerImpl(Context context, ConnectivityManager connectivityManager,
        ...
        // SPRD: modify for bug454715
        mSimStates = new String[mPhone.getPhoneCount()];
    ...
    private void registerListeners() {
        ...
        //SPRD:MODIFY FOR BUG 456545
        filter.addAction(ACTION_MODEM_ASSERT);
        // SPRD: Add HD audio icon in cucc for bug 509605.
        filter.addAction(TelephonyIntentsSprd.ACTION_HIGH_DEF_AUDIO_SUPPORT);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION_IMMEDIATE) ||
            ...
            // SPRD: modify for bug454715
            handleSimStateChangedForCarrierLabel(intent);

            // Might have different subscriptions now.
            updateMobileControllers();
        } else if (action.equals(ACTION_MODEM_ASSERT)){
            //SPRD: MODIFY FOR BUG 456545
            int length = mSignalClusters.size();
            for (int i = 0; i < length; i++) {
                mSignalClusters.get(i).setVolteIcon(false);
            }
        }
        /* SPRD: Add HD audio icon for bug 509605. @{ */
        else if (action.equals(TelephonyIntentsSprd.ACTION_HIGH_DEF_AUDIO_SUPPORT)) {
            boolean isHdVoiceSupport = intent.getBooleanExtra("isHdVoiceSupport", false);
            Log.d(TAG, "HIGH_DEF_AUDIO_SUPPORT = " + isHdVoiceSupport);
            int length = mSignalClusters.size();
            for (int i = 0; i < length; i++) {
                mSignalClusters.get(i).setHdVoiceIcon(isHdVoiceSupport);
            }
        /* @} */
        } else {
    ...
    private boolean hasNoSims() {
        int phoneCount = mPhone.getPhoneCount();
        for (int i = 0; i < phoneCount; i++) {
            if (!IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(mSimStates[i])) {
                return false;
            }
        }
        return true;
    ...
    protected void refreshCarrierLabelIfNoSim() {
        ...
        int color = SystemUIPluginsHelper.getInstance().getSubscriptionInfoColor(null);
    ...
    /* SPRD: bug 498633 modify for flight mode name display in reliance. @{ */
    ...
    private void immediatelyRefreshMobileIconIfSimExist(Intent intent) {
        int phoneId = intent.getIntExtra(PhoneConstants.PHONE_KEY, SubscriptionManager.DEFAULT_PHONE_INDEX);
        String simState = intent.getStringExtra(IccCardConstants.INTENT_KEY_ICC_STATE);
        ...
        if (!IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(simState)) {
            int length = mSignalClusters.size();
            for (int i = 0; i < length; i++) {
                if (tm.isSimStandby(phoneId)) {
                    mSignalClusters.get(i).updateMobileIconIfSimExist(phoneId);
                    mSignalClusters.get(i).isNeedSetColorfulIcon(phoneId);
                } else {
                    mSignalClusters.get(i).isNeedSetStandbyIcon(phoneId);
                }
            }
        }
    }
    ...
    void setCurrentSubscriptions(List<SubscriptionInfo> subscriptions) {
        ...
        HashMap<Integer, MobileSignalController> cachedControllers =
                new LinkedHashMap<Integer, MobileSignalController>(mMobileSignalControllers);
    ...
    private void updateConnectivity() {
        ...
        /* SPRD: Add for 430244 to get the current NetworkType which is connected @{ */
...
//        }
        TelephonyManagerSprd tm = (TelephonyManagerSprd) TelephonyManager.from(mContext);
        Log.d(TAG, "refreshCarrierLabel  mAirplaneMode="+mAirplaneMode);
        /* SPRD: bug 498633 modify for flight mode name display in reliance. @{ */
        if (true/*isRelianceBoard*/ && mAirplaneMode) {
            refreshCarrierLabelIfAirplaneMode();
            return;
        }

        // SPRD: mHasNoSims is not the real state use method named 'hasNoSims' instead.
        if (hasNoSims()) {
...
//        }
        String searchNet= Resources.getSystem().getText(com.android.internal.R.string.lockscreen_carrier_searching).toString();
       // String searchNet="searching for network";
        int phoneCount = mPhone.getPhoneCount();
        for (int i = 0; i < phoneCount; i++) {
            MobileSignalController controller = findMobileSignalControllerByPhoneId(i);
            if (controller != null) {
                label = controller.getLabel(label, mConnected, mHasMobileDataFeature);
            } else if (!IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(mSimStates[i])) {
                SubscriptionInfo subInfo = SubscriptionManager.from(mContext)
                        .getActiveSubscriptionInfoForSimSlotIndex(i);
                int color = SystemUIPluginsHelper.getInstance().getSubscriptionInfoColor(subInfo);
                if (DEBUG)
                    Log.d(TAG, "refreshCarrierLabel phoneId = " + i + "  color = " + color);
                SpannableStringBuilder noServiceString=null;
                if(!tm.isSimStandby(i)){
                    noServiceString = new SpannableStringBuilder(
                        Resources.getSystem().getText(com.android.internal.R.string.lockscreen_carrier_default).toString());
                }else{
                    noServiceString = new SpannableStringBuilder(searchNet);
                }
                noServiceString.setSpan(new ForegroundColorSpan(color), 0,
                        noServiceString.length(), SpannableStringBuilder.SPAN_EXCLUSIVE_EXCLUSIVE);
                label.append(noServiceString);
            }
        }
        /* @} */
    ...
    /* SPRD: modify for bug454715 @{ */
    private MobileSignalController findMobileSignalControllerByPhoneId(int phoneId) {
        for (MobileSignalController controller : mMobileSignalControllers.values()) {
            if (controller.mSubscriptionInfo.getSimSlotIndex() == phoneId) {
                return controller;
            }
        }
        return null;
    ...
    private final OnSubscriptionsChangedListener mSubscriptionListener =
        ...
        public void onSubscriptionsChanged() {
            updateMobileControllers();
            /* SPRD: immediately refresh carrier label if sim color changed,bug454715 @{ */
            for (MobileSignalController controller : mMobileSignalControllers.values()) {
                controller.updateSubscriptionInfo();
                /* SPRD: Add for Bug459030. @{ */
                boolean isSignalColorful= mContext.getResources().getBoolean(R.bool.enable_signal_strenth_color);
                if (isSignalColorful) {
                    controller.notifySubInfoColorChanged();
                }
                /* @} */
            }
            refreshCarrierLabel();
    ...
    static class WifiSignalController extends
        ...
        public void notifyListeners() {
            ...
            for (int i = 0; i < signalClustersLength; i++) {
                if (WIFI_SIGNAL_OPTIMIZED && mCurrentState.connected) {
    ...
    public static class MobileSignalController extends SignalController<
        ...
        /* @} */
        /* SPRD: Add for bug453070 @{ */
        private boolean mIsCMCCVolte = "cmcc".equals(SystemProperties.get("ro.operator.volte", ""));
        /* @} */
        ...
        public MobileSignalController(Context context, Config config, boolean hasMobileData,
           ...
           // mNetworkNameDefault="searching for network";//
            mNetworkNameDefault = Resources.getSystem().getText(com.android.internal.R.string.lockscreen_carrier_searching).toString();
        ...
        public void updateSubscriptionInfo() {
            ...
            SubscriptionInfo newSubInfo = SubscriptionManager.from(mContext).getActiveSubscriptionInfo(subId);
            if (newSubInfo != null) {
                mSubscriptionInfo = newSubInfo;
            }
        }
        /* @} */
         ...
         */
        public SpannableStringBuilder getLabel(SpannableStringBuilder currentLabel, boolean connected, boolean isMobileLabel) {
            SpannableStringBuilder ssb = new SpannableStringBuilder("");
            TelephonyManagerSprd tms = (TelephonyManagerSprd) TelephonyManager.from(mContext);
            if (!mCurrentState.enabled) {
                ...
                //SPRD: REMOVE FOR BUG 479268
//                if (currentLabel.length() != 0) {
//                    currentLabel = currentLabel.append(mNetworkNameSeparator);
//                }

                /* SPRD: bug419269 modify for network name display @{ */
                int color = SystemUIPluginsHelper.getInstance().getSubscriptionInfoColor(mSubscriptionInfo);
                if(!tms.isSimStandby(mSubscriptionInfo.getSimSlotIndex())){
                    mobileLabel=Resources.getSystem().getText(com.android.internal.R.string.lockscreen_carrier_default).toString();
                }
                Log.d(TAG, "getLabel[" + mSubscriptionInfo.getSimSlotIndex() + "]: " +
        ...
        private void mapIconSets() {
            ...
            if (mConfig.show4gForLte && !TelephonyManagerSprd.getVolteEnabled()) {
                mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_LTE, TelephonyIcons.FOUR_G);
            } else {
                /* SPRD: Add for bug453070 @{ */
                mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_LTE, mIsCMCCVolte ? TelephonyIcons.FOUR_G : TelephonyIcons.LTE);
                  /* @} */
            }
        }
        ...
        @Override
        public void notifyListeners() {
            // SPRD: See bug #454357. Use RPC call as less as possible.
            if (!SubscriptionManager.isValidPhoneId(mSubscriptionInfo.getSimSlotIndex())) {
                Log.d(TAG,
                        "  the SubscriptionId is INVALID: " + mSubscriptionInfo.getSubscriptionId());
                return;
            }
            ...
            /* @} */
            Log.d(TAG, "   showDataIcon:    " + showDataIcon);
            //showDataIcon = true;//HQ00121315,zhangduanhua,20160217
            ...
            // SPRD: show data icon only when in service
            int typeIcon = (showDataIcon && hasService()) ? icons.mDataType : 0;
            ...
            if (isRelianceBoard) {
                isFourG = isFourGLTE();
                if (hasService() && isFourGLTE() && !isCSFB() && mImsState == 1) {
                    imsregIcon = SystemUIPluginsHelper.getInstance().getSignalVoLTEIcon();
                }
            }
            /* @} */
            int signalClustersLength = mSignalClusters.size();
            int mobileDataInId = mCurrentState.dataSim && mCurrentState.dataConnected && mCurrentState.activityIn ? R.drawable.ic_qs_signal_in
                    : 0;
            int mobileDataOutId = mCurrentState.dataSim && mCurrentState.dataConnected && mCurrentState.activityOut ? R.drawable.ic_qs_signal_out
                    : 0;
            /* SPRD: add data activity display in status bar @{ */
            /* SPRD: Modify for bug 430244 to hide the data traffic icon
               when it is not default network. @{ */
            int currentNetworkType = mNetworkController.getConnectedNetworkType();
            /* SPRD: add for reliance case @{ */
            if (mCurrentState.dataConnected) {
                if (mCurrentState.activityIn && mCurrentState.activityOut) {
                    mobileDataInId = SystemUIPluginsHelper.getInstance().getDataInOutIcon();
                } else if (mCurrentState.activityIn) {
                    mobileDataInId = SystemUIPluginsHelper.getInstance().getDataInIcon();
                } else if (mCurrentState.activityOut) {
                    mobileDataInId = SystemUIPluginsHelper.getInstance().getDataOutIcon();
                } else {
                    if (currentNetworkType != -1
                            && currentNetworkType != ConnectivityManager.TYPE_MOBILE) {
                        mobileDataInId = 0;
                    } else {
                        mobileDataInId = SystemUIPluginsHelper.getInstance().getDataDefaultIcon();
                    }
                }
            } else {
                mobileDataInId = 0;
            ...
            Log.d(TAG,
                    "  mSubscriptionInfo.getSubscriptionId() "
                            + mSubscriptionInfo.getSubscriptionId() + "  typeIcon: " + typeIcon
                            + "  mobileDataInOutId: " + mobileDataInId+ "   "+mobileDataOutId);
            /* SPRD: Modify for bug 434412 @{ */
            for (int i = 0; i < signalClustersLength; i++) {
                /* SPRD: Reliance UI spec 1.7. See bug #503821. @{ */
                if (isRelianceBoard) {
                    mSignalClusters.get(i).setMobileDataIndicators(
                            ...
                            mSubscriptionInfo.getSubscriptionId(),mobileDataInId,mobileDataOutId);
                } else {
                    mSignalClusters.get(i).setMobileDataIndicators(
                            mCurrentState.enabled && !mCurrentState.airplaneMode,
                            getCurrentIconId(),
                ...
                /* @} */
                mSignalClusters.get(i).isNeedSetStandbyIcon(mSubscriptionInfo.getSimSlotIndex());
            }
            /* @} */
        }

        public void notifySubInfoColorChanged() {
            int signalClustersLength = mSignalClusters.size();
            for (int i = 0; i < signalClustersLength; i++) {
            mSignalClusters.get(i).isNeedSetColorfulIcon(mSubscriptionInfo.getSimSlotIndex());
            }
        }
        ...
        public int getCurrentIconId() {
            int[][] colorfulSbIcons = SystemUIPluginsHelper.getInstance()
                    .getColorfulSignalStrengthIcons(mSubscriptionInfo.getSubscriptionId());
            if (colorfulSbIcons != null) {
                getIcons().mSbIcons = colorfulSbIcons;
            }
        ...
        private boolean isFourGLTE() {
            if (mServiceState != null) {
                return mServiceState.getRilDataRadioTechnology() == ServiceState.RIL_RADIO_TECHNOLOGY_LTE;
            } else {
                return false;
            }
        }
        ...
        private boolean isCSFB() {
            return TelephonyManagerSprd.getRadioCapbility()
        ...
        void updateNetworkName(boolean showSpn, String spn, boolean showPlmn, String plmn) {
            ...
            if(isRelianceBoard){
                ...
                if (showSpn && spn != null && !spn.equals(plmn)) {
                      str.append(spn);
                }
            }
            if (str.length() != 0) {
            ...
            String networkName = SystemUIPluginsHelper.getInstance().updateNetworkName(
                    mContext, showSpn, spn, showPlmn, plmn, phoneId);
        ...
        class MobilePhoneStateListener extends PhoneStateListener {
            ...
            public void onVoLteServiceStateChanged(VoLteServiceState stateInfo) {
                Log.d(mTag, "onVoLteServiceStateChanged : ImsState = " + stateInfo.getImsState());
                if(mImsState != stateInfo.getImsState()) {
                    mImsState = stateInfo.getImsState();
                    int length = mSignalClusters.size();
                    for (int i = 0; i < length; i++) {
                        mSignalClusters.get(i).setVolteIcon(mImsState == 1);
                       }
                    // SPRD: modify for bug 497225
                    if (isRelianceBoard) {
                        notifyListeners();
                    }
                }
    ...
    public interface SignalCluster {
        ...
        void setWifiIndicators(boolean visible, int strengthIcon, boolean wifiIn, boolean wifiOut, String contentDescription);

        // SPRD: modify for bug 434412
        void setMobileDataIndicators(boolean visible, int strengthIcon, int typeIcon, int roamIcon,
                String contentDescription, String typeContentDescription, boolean isTypeIconWide,
                int subId, int mobileDataInId, int mobileDataOutId);
        // SPRD: Reliance UI spec 1.7. See bug #503821.
        void setMobileDataIndicators(boolean visible, int strengthIcon, int typeIcon, int roamIcon,
                int imsregIcon, boolean isFourG,
                String contentDescription, String typeContentDescription, boolean isTypeIconWide,
                int subId, int mobileDataInId, int mobileDataOutId);
        void setSubs(List<SubscriptionInfo> subs);
        void setNoSims(boolean show);
        // SPRD: modify for bug454715
        void updateMobileIconIfSimExist(int phoneId);
        void isNeedSetStandbyIcon(int phoneId);
        void isNeedSetColorfulIcon(int phoneId);

        void setIsAirplaneMode(boolean is, int airplaneIcon, int contentDescription);
        /* SPRD: bug450508 modify for volte icon @{ */
        void setVolteIcon(boolean enabled);
        /* @} */
        // SPRD: Add HD audio icon for bug 509605.
        void setHdVoiceIcon(boolean enabled);
    }
    ...
    static class Config {
        ...
        static Config readConfig(Context context) {
            ...
            config.hspaDataDistinguishable = /*res.getBoolean(R.bool.config_hspa_data_distinguishable)*/
                    SystemUIPluginsHelper.getInstance().hspaDataDistinguishable();
            /* @} */
            return config;
        }
    }
